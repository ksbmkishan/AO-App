...
const ChatIntakeForm = ({
  ...
  const [items, setItems] = useState([
    { label: "Select Marital", value: "Select Marital" },
  ...
  console.log('CheckLinkedProfileData:::kKK', linkedProfileData)
  useEffect(() => {
    ...
    return (
      <View style={styles.itemRowContainer}>
        ...
        <View style={{ flex: 0.5, marginBottom: 15 }}>
          ...
          <DropDownPicker
            open={open}
            value={maritalStatus}   // from your state object
            items={items}
            setOpen={setOpen}
            setItems={setItems}
            setValue={(callback) => {
              // callback is the new value
              const value = callback(maritalStatus);
              console.log("Selected marital status:", value);

              setState((prev) => ({
                ...prev,
                maritalStatus: value,
                ...(newProfile ? { newProfile: false, selectedProfile: null } : {})
              }));
            }}
...
function genderInfo() {
  ...
  return (
    <View style={styles.itemContainer}>
      ...
      <DropDownPicker
        open={open}
        value={gender} // 👈 comes from your state
        items={items}
        setOpen={setOpen}
        setItems={setItems}
        setValue={(callback) => {
          updateState({ gender: callback(gender) });
          if (newProfile) {
            updateState({ newProfile: false, selectedProfile: null });
          }
        }}
