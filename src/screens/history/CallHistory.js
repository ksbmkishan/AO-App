    ...
    return (
      <View style={styles.container}>
        <Text
          ...
          {t("ORDER ID")}: {item?.transactionId.slice(-1 - 10)}
        </Text>
        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
          ...
          <View style={{ marginLeft: Sizes.fixPadding }}>
            ...
            <Text style={{ ...Fonts.gray12RobotoMedium }}>
              <TranslateText title={item?.astrologerDetails?.gender} />
            </Text>
          </View>
        </View>
        <View style={{ marginTop: Sizes.fixPadding }}>
          <Text
            ...
            {t("Order Time")}: {moment(item?.createdAt).format('DD/MM/YYYY hh:mm A')}
          </Text>
          <Text
            style={{ ...Fonts.gray14RobotoRegular, color: Colors.blackLight }}>
            {t("Duration")}: {secondsToHMS(item?.durationInSeconds)}
          </Text>
          <Text
            style={{ ...Fonts.gray14RobotoRegular, color: Colors.blackLight }}>
            {t("Call Price")}: {showNumber(item?.callPrice)}/min
          </Text>
          <Text
            style={{ ...Fonts.gray14RobotoRegular, color: Colors.blackLight }}>
            {t("Total Charge")}: {showNumber(item?.totalCallPrice)}
          </Text>
          <Text
            style={{ ...Fonts.gray14RobotoRegular, color: Colors.blackLight }}>
    ...
    <View style={{ flex: 1, backgroundColor: Colors.whiteDark }}>
      {callHistoryData ?
        ...
        : <View style={{ display: "flex", justifyContent: "center", alignItems: "center", paddingVertical: 200 }}>
          <Text style={{ display: "flex", alignSelf: "center", justifyContent: "center", color: colors.black_color, fontSize: 15 }}>{t("No Data Available")}</Text>
        </View>
      }
    </View>
  );
