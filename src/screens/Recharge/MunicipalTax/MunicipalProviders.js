...
const MunicipalProviders = ({ dispatch, navigation, allServiceProviders, rechargeBanner }) => {
  ...
  function renderList() {
    const renderItem = ({ item }) => (
      <TouchableOpacity
        onPress={() =>
          navigation.navigate("MunicipalTaxPageData", { providerData: item })
        }
        activeOpacity={0.8}
  ...
  function titleText() {
    return (
      ...
      >
        <Text style={styles.title}>{t("Municipal Tax Providers")}</Text>
      </View>
    );
  }
    ...
    return (
      <View style={styles.searchBox}>
        <TextInput
          ...
          placeholder={t("Search Municipal Provider")}
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.white }}>
      ...
      />
      <MyHeader
        title={t("Select Municipal Provider")}
...
export default connect(mapStateToProps)(MunicipalProviders);
