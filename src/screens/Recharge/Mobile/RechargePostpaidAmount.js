...
const RechargePostpaidPayment = ({ route, customerData, dispatch, billInfo, navigation }) => {
        ...
        requestPermission().then((hasPermission) => {
            if (hasPermission) {
                const payload = {
                    ...
                    latitude: 28.4521,
                    longitude: 77.5241,
                    billType: 'Recharge-postpaid',
                    bill_fetch_ref: billInfo?.order_id,
                     serviceType: 'POSTPAID RECHARGE',
                    dispatch: dispatch
                }
    ...
    return (
        <SafeAreaView style={styles.safeArea}>
            ...
            <MyHeader title={'Recharge Postpaid'} tintColor={Colors.white} navigation={navigation} color='#002E6E'/>
        ...
        return (
            <View style={styles.vehicleNumberContainer}>
                <View style={styles.vehicleImageContainer}>
                    <Image source={providerData?.operatorImage} style={styles.vehicleImage} />
                </View>
            ...
            <BottomSheet isVisible={detailModal} onBackdropPress={() => setDetailModal(false)}>
                <View style={styles.maincontainer}>
                    <View style={styles.bottomSheetHeader}>
                        <Text style={styles.bottomSheetTitle}>Details</Text>
                        <View></View>
                        <TouchableOpacity onPress={() => setDetailModal(false)}>
                            <AntDesign name="closecircleo" color={Colors.black} size={Sizes.fixPadding * 1.7} />
                        </TouchableOpacity>
                    </View>
                    {renderDetailRow('Name', billInfo?.data?.customer_name ?? 'N/A')}
                    {renderDetailRow('Bill Number', billInfo?.order_id)}
                    {renderDetailRow('Amount Before Due Date', billInfo?.data?.amount)}
                    {renderDetailRow('Consumer Name', billInfo?.data?.customer_name ??  'N/A')}
                    {renderDetailRow('Number', providerData?.number)}
                    {renderDetailRow('Operator Name', providerData?.productName)}
                </View>
            </BottomSheet>
        );
    }
...
export default connect(mapStateToProps, mapDispatchToProps)(RechargePostpaidPayment);
