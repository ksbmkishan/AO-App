...
const WaterPage = ({ dispatch, navigation, allServiceProviders, rechargeBanner }) => {
  ...
  function renderList() {
    const renderItem = ({ item }) => (
      <TouchableOpacity
        onPress={() =>
          navigation.navigate("WaterBillPage", { providerData: item })
        }
        activeOpacity={0.8}
  ...
  function titleText() {
    return (
      ...
      >
        <Text style={styles.title}>Water Bill Providers</Text>
      </View>
    );
  }
    ...
    return (
      <View style={styles.searchBox}>
        <TextInput
          ...
          placeholder="Search Water Bill Provider"
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.white }}>
      ...
      />
      <MyHeader
        title={"Select Water Provider"}
        tintColor={Colors.white}
        navigation={navigation}
        color="#002E6E"
      />
      {/* Carousel */}
      <View style={{ height: SCREEN_WIDTH / 2.2, borderRadius: 10 , padding:10, overflow:'hidden'}}>
        <Carousel
          loop
          width={SCREEN_WIDTH}
...
export default connect(mapStateToProps)(WaterPage);
...
const styles = StyleSheet.create({
  ...
  itemText: {
    ...
    fontSize: 14,
    flexShrink: 1, // taaki text wrap ho
  },
  title: {
