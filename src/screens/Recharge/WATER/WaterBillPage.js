...
const WaterBillPage = ({ route, dispatch, rechargeRequestFields, rechargeBanner }) => {
    ...
    const validate = () => {
        ...
        if (!customerNumber) {
            err.customerNumber = 'Invalid Customer Number';
            valid = false;
        }
            ...
            requestPermission().then((hasPermission) => {
                if (hasPermission) {
                    const payload = {
                        ...
                        onComplete: () => navigation.navigate('WaterPayment', {
                            providerData: {
                                number: customerNumber,
                    ...
                    console.log('CheckPayloadfroWater:::',payload)
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }}>
            <MyHeader title={'Water Bill Payment'} tintColor={Colors?.white} navigation={navigation} color='#002E6E'/>
            <MyLoader />
            <ScrollView contentContainerStyle={styles.scrollContainer}>
                <View style={{ flex: 1, padding: Sizes.fixPadding }}>
                    ...
                    {/* Operator Info */}
                    <TouchableOpacity style={styles.inputWrapper} onPress={() => navigation.navigate('WaterPage')}>
...
export default connect(mapStateToProps, mapDispatchToProps)(WaterBillPage);
