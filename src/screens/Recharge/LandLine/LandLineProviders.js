...
const LandLineProviders = ({ dispatch, navigation, allServiceProviders, rechargeBanner }) => {
  const [search, setSearch] = useState("");
  const [filteredData, setFilteredData] = useState([]);
  const {t} = useTranslation();
  useEffect(() => {
    const payload = {
      serviceId: 16, // Landline ka serviceId
    };
  ...
  function renderList() {
    const renderItem = ({ item }) => (
      <TouchableOpacity
        onPress={() =>
          navigation.navigate("LandLineBillPage", { providerData: item })
        }
        activeOpacity={0.8}
  ...
  function titleText() {
    return (
      ...
      >
        <Text style={styles.title}>{t("Landline Providers")}</Text>
      </View>
    );
  }
    ...
    return (
      <View style={styles.searchBox}>
        <TextInput
          ...
          placeholder={t("Search Landline Provider")}
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.white }}>
      ...
      />
      <MyHeader
        title={t("Select Landline Provider")}
...
export default connect(mapStateToProps)(LandLineProviders);
