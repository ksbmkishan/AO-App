    ...
    return (
      <TouchableOpacity
        ...
        <View style={{ flex: 0, flexDirection: 'row' }}>
          <Image
            source={item?.image?.length != 0 ? { uri: img_url + item?.image } : require('../../assets/icons/astroone.png')}
            style={{
              width: width * 0.18,
              height: width * 0.18,
              borderRadius: 1000,
  ...
  function NoDATA() {
    return (
      <View style={{ justifyContent: 'center', alignItems: "center", paddingVertical: SCREEN_HEIGHT * 0.35 }}>
        <Text style={{ ...Fonts.black11InterMedium, fontSize: 17 }}>{t("No Notification Found")}</Text>
