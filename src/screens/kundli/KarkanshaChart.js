...
import ShowSvg from './components/ShowSvg2';
...
const KarkanshaChart = ({ basicDetails, dispatch, karkanshaChartData , isLoading}) => {
  ...
  useEffect(() => {
    ...
    dispatch(KundliActions.getKarkanshaChartData(payload));
  }, [dispatch]);

  console.log('CheckChartData::KK', karkanshaChartData);

  if (isLoading || !karkanshaChartData) {
              return <Loader  />;
          }
  ...
  return (
    <View>
      ...
      <KarkanshaChartSvg data={karkanshaChartData} />
      </View>
...
const mapStateToProps = state => ({
  ...
  karkanshaChartData: state.kundli.karkanshaChartData,
});

const mapDispatchToProps = dispatch => ({ dispatch });

export default connect(mapStateToProps, mapDispatchToProps)(KarkanshaChart);
