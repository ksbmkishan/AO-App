...
const TrishamanshaChart = ({ basicDetails, dispatch, TrishamanshaChart,isLoading }) => {

    console.log("arpit", TrishamanshaChart)
    ...
    useEffect(() => {
        ...
        dispatch(KundliActions.getTrishamanshaChart(payload))
    }, [dispatch])

  if (isLoading || !TrishamanshaChart) {
              return <Loader  />;
          }
        ...
        <View>
           {/* <View style={{ alignItems: "flex-end", elevation: 1, paddingHorizontal: SCREEN_WIDTH * 0.02, paddingVertical: SCREEN_HEIGHT * 0.01, backgroundColor: colors.mybackground, borderRadius: 5, gap: 2 }}>
            ...
            <View style={{paddingVertical:SCREEN_HEIGHT*0.03}}>
                <ChartSvg data={TrishamanshaChart} />
            </View>
...
const mapStateToProps = state => ({
    ...
    TrishamanshaChart: state.kundli.TrishamanshaChart,
...
export default connect(mapStateToProps, mapDispatchToProps)(TrishamanshaChart);
