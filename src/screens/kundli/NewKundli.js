...
const NewKundli = ({ customerData, navigation, locationData, dispatch, isLoading, kUNDLIRESPONSE }) => {
  console.log("kUNDLIRESPONSE", kUNDLIRESPONSE)
  ...
  useEffect(() => {
    ...
    return () => {
      dispatch(KundliActions.setKundliId(kUNDLIRESPONSE?._id));
  ...
  const create_kundli = async () => {
    if (name.length == 0) {
      ...
      dispatch(KundliActions.getcreateKundli(payload))
      ...
      <View style={styles.container}>
        ...
        <TouchableOpacity
          ...
          <MaterialCommunityIcons
            ...
            size={getFontSize(2)}
          />
          <Text allowFontScaling={false} style={{ ...FontsStyle.font, }}>
...
const mapStateToProps = state => ({
  ...
  kUNDLIRESPONSE: state.kundli.kUNDLIRESPONSE,
});
