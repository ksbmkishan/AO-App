...
const HealthStone = ({ basicDetails, dispatch, DataHealth, MYlagnaCHART }) => {
    const { i18n } = useTranslation();

    const health = MYlagnaCHART?.chart?.[0]?.rashiIndex;
    ...
    const isLoadingData =
        !MYlagnaCHART || !MYlagnaCHART.chart || !DataHealth?.data || !health;
                ...
                <View style={styles.contentWrapper}>
                    <View style={styles.card}>
                        ...
                        <View style={styles.row}>
                            ...
                            <Text style={styles.value}>{DataHealth?.data?.zodiac}</Text>
                        </View>

                        <View style={styles.row}>
                          <Text style={styles.label}>{t("Ruling Planet")}:</Text>
                            <Text style={styles.value}>{DataHealth?.data?.rulingPlanet}</Text>
                        </View>

                        <View style={{ ...styles.row }}>
                             <Text style={[styles.label, { marginTop: 10 }]}>{t("Destiny/Luck Stone")}:</Text>
                            <Text style={styles.value}>{DataHealth?.data?.gemstone}</Text>
                        </View>

                         <Text style={[styles.label, { marginTop: 10 }]}>✅ {t("Benefits")}:</Text>
                        <Text style={styles.highlightValue}>{DataHealth?.data?.benefits}</Text>

                        <Text style={[styles.label, { marginTop: 10 }]}>👤 {t("Who Should Wear")}:</Text>
                        <Text style={styles.highlightValue}>{DataHealth?.data?.whoShouldWear}</Text>

                        <Text style={[styles.label, { marginTop: 10 }]}>🌟 {t("Famous Personalities Wearing")} {DataHealth?.data?.gemstone}:</Text>
                        {DataHealth?.data?.famousPersonalities?.map((name, index) => (
                            <Text key={index} style={styles.highlightValue}>
                                • {name}
                            </Text>
                        ))}

                        <Text style={[styles.label, { marginTop: 10 }]}>✨ {t("Unique Insight About")} {DataHealth?.data?.gemstone}:</Text>
                        <Text style={styles.highlightValue}>{DataHealth?.data?.uniqueInsight}</Text>

                        <Text style={[styles.label, { marginTop: 10 }]}>❓{t("Unknown Fact")}:</Text>
                        <Text style={styles.highlightValue}>{DataHealth?.data?.unknownFact}</Text>

                         <Text style={[styles.label, { marginTop: 10 }]}>🔍 {t("How to Identify Whether It’s Real")}:</Text>
                        <Text style={styles.highlightValue}>{DataHealth?.data?.howToIdentifyWhetherItReal}</Text>
...
const mapStateToProps = (state) => ({
    ...
    DataHealth: state.kundli.DataHealth,
