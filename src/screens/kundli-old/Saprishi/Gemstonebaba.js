...
const Gemstonebaba = ({ navigation, route, dispatch, isLoading, basicDetails }) => {
    const { t } = useTranslation();
    const viewRef = useRef(null);

    const [show, setShow] = useState('LuckyGemstone');
    const kundliId =route?.params?.kundliId
    console.log("kundliIdgem",kundliId)
    ...
    function tabsInfo() {
        ...
        const tabs = [
            ...
            { key: 'LuckyGemstone', label: t('LUCKY CHARM') },
...
export default connect(mapStateToProps, mapDispatchToProps)(Gemstonebaba);
