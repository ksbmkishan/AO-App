...
const EnterKundli = ({ customerData, navigation, route, dispatch, locationData, subLocationData }) => {
  ...
  const get_matching = async (mid, fid) => {
    if (validation()) {
      ...
      const payload = {
        femaleKundliData: femaleKundliData, maleKundliData: maleKundliData, matchingPayload: matchingPayload, customerId: customerData?._id
      }
      ...
      dispatch(KundliActions.onKundliMatching(payload))
