...
import { api_url, book_pooja, customer_home_banner, get_all_pooja_list, get_All_Religion, get_All_Religion_SubCategory, get_call_astrologer, get_chat_astrologer, get_new_pooja, get_pooja, get_puja_details, get_puja_history_data, get_video_call_astrologer, phonepeWallet } from '../../config/constants';
...
function* getReligionCollection(actions) {
    try {
        yield put({ type: actionTypes.SET_IS_LOADING, payload: true })
        const response = yield getRequest({
            url: api_url + get_All_Religion,
        })


        if (response?.success) {
            yield put({ type: actionTypes.SET_ALL_RELIGION_COLLECTION, payload: response?.data })
        }
...
function* getReligionCollectionSubCategory(actions) {
    const { payload } = actions
    console.log('checkPayload:::KK', payload)
    try {
        yield put({ type: actionTypes.SET_IS_LOADING, payload: true })
        const Response = yield postRequest({
            url: api_url + get_All_Religion_SubCategory,
            data: { categoryId: payload?.categoryId }
        })
        // console.log("111222", Response)
        if (Response?.success) {
            yield put({ type: actionTypes.SET_ALL_RELIGION_COLLECTION_SUB_CATGORY, payload: Response?.data })
        } else {
            Alert.alert("Astro One", Response?.message);
        }
    } catch (e) {
        yield put({ type: actionTypes.SET_IS_LOADING, payload: true })
        console.log('hii', e);
...
export default function* poojaSaga() {
    ...
    yield takeLeading(actionTypes.GET_ALL_RELIGION_COLLECTION, getReligionCollection)
    yield takeLeading(actionTypes.GET_ALL_RELIGION_COLLECTION_SUB_CATGORY, getReligionCollectionSubCategory)
