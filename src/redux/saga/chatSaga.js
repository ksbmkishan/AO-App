...
function* onChatRequestSend(actions) {
    ...
    try {
        ...
        if (type == 'chat') {
            ...
            yield put({
                type: actionTypes.SET_NEW_LINKED_DATA,
                payload: send_request?.linkedProfile,
            });
...
function* onAcceptRejectChat(actions) {
    try {
        ...
        const linkedData = yield select((state) => state.chat.linkedData);
          const formattedDOB = linkedData?.dateOfBirth ? new Date(linkedData?.dateOfBirth).toLocaleDateString('en-GB') : 'N/A';
        const formattedTime = linkedData?.timeOfBirth ? new Date(linkedData?.timeOfBirth).toLocaleTimeString('en-GB') : 'N/A';

        const customerData = yield select((state) => state.customer.customerData);

        console.log(' STATUS ::::::::::: ', status, requestedData);
        ...
        if (status == 'accept') {
            ...
            const msg = {
                ...
                text: `Hi, 
                    ...
                    Name: ${linkedData?.firstName}
                    Gender: ${linkedData?.gender}
                    DOB: ${formattedDOB}
                    Time: ${formattedTime}
                    Address: ${linkedData?.placeOfBirth}`,
                user: {
            ...
            yield put({ type: actionTypes.SAVE_CHAT_MESSAGE, payload: msg });
            yield* customerMessage(msg);
...
function* customerMessage(payload) {
  try {
    ...
    // const responseSend = yield postRequest({
    //   url: api_url + 'customers/chat-send-message-notification',
    //   data: {
    //     astroId: requestedData?.astroID,
