...
import { api_url, get_all_astro_magazine, get_all_auspicious_time, get_all_birhat_horoscope, get_all_daily_panchang, get_all_religion_spirituality, get_all_remedies, get_all_yellow_book, get_app_tutorials, get_astro_blogs, get_blogs_by_category, get_Testmonial_Data } from '../../config/constants'
...
function* getTestmonialData(actions) {
    try {
        yield put({ type: actionTypes.SET_IS_LOADING, payload: true })
        const response = yield getRequest({
            url: api_url + get_Testmonial_Data
        })

        if (response?.success) {
            yield put({ type: actionTypes.SET_TESTMONIAL_DATA, payload: response?.testimonial })
        }
...
function* getAstroBlogsCategory(actions) {
    try {
        yield put({ type: actionTypes.SET_IS_LOADING, payload: true })
        const response = yield getRequest({
            url: api_url + get_blogs_by_category
        })
        console.log('response ::: ',response);
        if (response?.success) {
            yield put({ type: actionTypes.SET_ASTRO_BLOGS_CATEGORY, payload: response?.categoryBlog })
        }
...
export default function* blogSaga() {
    ...
    yield takeLeading(actionTypes.GET_TESTMONIAL_DATA, getTestmonialData)
    yield takeLeading(actionTypes.GET_ASTRO_BLOGS_CATEGORY, getAstroBlogsCategory)
